export interface Order {
  ...
  delivered_at?: string | number | Date | null;
...
function parseOrderDate(order: Order): Date | null {
  const rawDate = order?.delivered_at ?? order?.created_at;
...
export function getRevenueChart(orders: Order[] | null | undefined): RevenueDatasets {
  ...
  for (let i = 6; i >= 0; i--) {
    const d = new Date(); d.setDate(now.getDate() - i); sevenDaysMap[d.toDateString()] = 0;
  }
  for (let i = 29; i >= 0; i--) {
    const d = new Date(); d.setDate(now.getDate() - i); thirtyDaysMap[d.toDateString()] = 0;
  }
  ...
  orders.forEach((order) => {
    if (order && isDelivered(order) && typeof order.vendor_earning === 'number') {
      ...
      if (orderDate.getFullYear() === now.getFullYear()) monthlyMap[months[orderDate.getMonth()]] += earning;
    }
  });

  result.sevenDays = Object.keys(sevenDaysMap).map(key => ({ label: weekdays[new Date(key).getDay()], value: sevenDaysMap[key] }));
  result.thirtyDays = Object.keys(thirtyDaysMap).map(key => ({ label: new Date(key).toLocaleDateString(undefined, { day: 'numeric', month: 'short' }), value: thirtyDaysMap[key] }));
  result.monthly = months.map(m => ({ label: m, value: monthlyMap[m] }));
  return result;
}

export function getOrderStatusBreakdown(orders: Order[] | null | undefined): StatusBreakdown {
  const breakdown: StatusBreakdown = { Pending: 0, Accepted: 0, Preparing: 0, Packed: 0, "Out For Delivery": 0, Delivered: 0, Cancelled: 0 };
...
export function getVendorCommission(orders: Order[] | null | undefined): number {
  if (!orders || !Array.isArray(orders)) return 0;
  return orders.reduce((sum, order) => sum + (order && typeof order.vendor_commission === 'number' ? order.vendor_commission : 0), 0);
}
...
export function getPlatformRevenue(orders: Order[] | null | undefined): number {
  ...
  return orders.reduce((sum, order) => {
    if (!order) return sum;
    return sum + (order.vendor_commission ?? 0) + (order.platform_fee ?? 0) + (order.rivo_delivery_margin ?? 0);
  }, 0);
}

export function getRiderPayout(orders: Order[] | null | undefined): number {
  if (!orders || !Array.isArray(orders)) return 0;
  return orders.reduce((sum, order) => sum + (order?.rider_earning ?? 0), 0);
}