...
export function Products({
  ...
  /*
   * IMPORTANT:
   * Categories are NOT hard-coded here.
   *
   * product_categories is the single source of truth.
   *
   ...
   *
   * Categories come directly from product_categories.
   * No hard-coded category IDs or names are used for data storage.
   */
  const fetchLiveProducts = async () => {
    try {
       ...
       *
       * display_order is used so every section of the portal
       * receives categories in the same database-defined order.
       */
      ...
      if (data) {
        const mapped = data.map((p: any) => {
          ...
          if (currentStock === 0) {
            ...
            currentStock <= Number(p.low_stock_threshold ?? 5)
          ) {
          ...
          return {
            ...
            /*
             * Keep the actual database category UUID.
             */
  ...
  /*
   * Normalize a CSV category name for matching only.
   *
   * IMPORTANT:
   * This does NOT convert one category into another.
   *
   * Example:
   * "Beverages" -> "beverages"
   * " beverages " -> "beverages"
  ...
  /*
   * Find the canonical category from product_categories.
   */
  ...
  const handleImportCSV = async () => {
    ...
    try {
      ...
      if (!authData?.user) {
        throw new Error("Authentication context invalid.");
      }
      ...
      if (vendorErr || !vendor) {
        throw new Error("Vendor system mismatch.");
      }

      /*
       * Make sure categories are available before processing
       * the CSV.
       *
       * This prevents category matching against stale/empty state.
       */
      ...
      if (availableCategories.length === 0) {
        ...
        if (freshCategoryError) {
          throw freshCategoryError;
        }

        availableCategories = (freshCategories || []).map(
          (category) => ({
            id: category.id,
            name: category.name.trim(),
          })
      ...
      const { data: currentDbProducts, error: dbProdErr } =
        await supabase
          .from("products")
          .select(
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (lines.length < 2) {
            alert(
              "The uploaded file does not contain any entries."
            );
            setIsImporting(false);
            return;
          }
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            if (
              localSeenInCsv.has(lowerCasedName)
            ) {
              continue;
            }
            ...
            const pStock =
              parseInt(
                rowData["stock"] ||
                  rowData["stock quantity"]
              ) || 0;

            /*
             * Existing products are restocked according
             * to the existing inventory rules.
             */
            if (
              ...
              if (
                match.stock === 0 ||
                match.stock <=
                  match.low_stock_threshold
              ) {
                productsToRestock.push({
                  id: match.id,
                  stock: pStock,
                });
             ...
             *
             * Priority 1:
             * Use category_id if the CSV explicitly contains
             * a valid canonical category UUID.
             *
             * Priority 2:
             * Otherwise match the CSV category name directly
             * against product_categories.name.
             *
             * There is NO category conversion/mapping.
             */
            let pCatId:
              | string
              | undefined =
              rowData["category_id"]?.trim() ||
              undefined;
            ...
            /*
             * Never insert a product with an invalid or
             * missing category.
             */
            if (!pCatId) {
            ...
            const pPrice =
              parseFloat(
                rowData["price"] ||
                  rowData["selling price"]
              ) || 0;

            const pCost =
              parseFloat(
                rowData["cost_price"] ||
                  rowData["wholesale rate"] ||
                  rowData["cost"]
              ) || 0;
          ...
          if (productsToInsert.length > 0) {
            ...
            if (insertErr) {
              if (
                ...
                alert(
                  "A uniqueness error occurred while processing bulk ingestion records."
                );
          ...
          /*
           * Restock existing low/out-of-stock products.
           */
          for (const item of productsToRestock) {
            ...
            if (updateErr) {
              console.error(
                `Failed to update stock for product ID ${item.id}:`,
                updateErr
              );
            }
          }
          ...
          alert(
            `Import Summary:\n` +
              ...
              (skippedCategoryCount > 0
                ? `• Skipped Missing/Invalid Categories: ${skippedCategoryCount}`
                : "")
          );
      ...
      console.error(
        "CSV Ingestion Pipeline Dropped:",
        err
      );
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] bg-[#10B981] text-white px-1.5 py-0.5 rounded-full font-medium">
                        Recommended
                      </span>
                    </h3>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      ...
                      bills, or receipts. The
                      system will extract products
              ...
              <div className="border border-border rounded-xl p-4 bg-muted/20 hover:bg-muted/40 transition-colors flex flex-col justify-between">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Import products using a
                      standard CSV spreadsheet
                      format.
                    </p>
                  </div>
                </div>
          ...
          <div className="relative bg-card rounded-2xl border border-border w-full max-w-md p-6 shadow-xl">
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <button
                  type="button"
                  className="mt-3 px-4 py-1.5 rounded-lg bg-[#10B981] text-white text-xs font-medium"
                >
                  Choose File
                </button>
              </div>
